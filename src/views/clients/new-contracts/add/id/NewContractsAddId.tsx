...
const NewContractsAddId = () => {
	...
	const { data: application, isLoading: isLoadingApplication } = useQuery({
		queryKey: ['get application', params.id],
		queryFn: () => ContractService.getApplicationById<ContractApplication>(Number(params.id)),
		select: ({ data }) => data
	})

	const { data: newContract, isLoading: isLoadingNewContract } = useQuery({
		queryKey: ['get new contract', application],
		queryFn: () => ContractService.getNewContract<DefaultContract>(),
		select: ({ data }) => data,
		enabled: !!application
	})
	...
	const { data: techs, isLoading: isLoadingTechs } = useQuery({
		queryKey: ['get techs', application],
	...
	const { data: branches, isLoading: isLoadingBranches } = useQuery({
		queryKey: ['get branches', application],
	...
	const onSubmit: SubmitHandler<FormModel> = async (data) => {
		...
		if (!application || !newContract || !customer || !metric) {
			toast.push(
		...
		const pdfBlob = await pdf(
			<ContractDocument
				...
				customer={customer}
				metric={metric}
				tech={application ? application.tech : selectedTech}
				tableData={tableData}
				totals={totals}
			/>
		).toBlob()
		...
		formData.append('contract_application', String(data.contract_application.id))
	...
	useEffect(() => {
		if (application && newContract && metric) {
			const price_with_vat = Number(application.tech.tech_price_with_vat)
			const price_with_gps = price_with_vat + Number(metric.gps) * (1 + Number(metric.vat) / 100)
			...
			reset({
				...
				region: application.branch.region ? application.branch.region.id : REGION_ID,
				branch_region: application.branch.region.name_latin,
				branch_city: application.branch.city.name_uz,
				branch_street: application.branch.street,
				branch_house: application.branch.house_number,
				...
				tech: application.tech.id,
				tech_model: application.tech.model_name_uz,
				tech_type: application.tech.type.name_uz,
				tech_manufacturer: application.tech.manufacturer.name_uz,
				deposit_percentage: String(Number(metric.min_deposit_percentage).toFixed(0)),
				price_with_vat: String(price_with_vat.toFixed(0)),
				price_with_vat_in_words: numToWordUz(price_with_vat.toFixed(0)),
				price_with_gps: String(price_with_gps.toFixed(0)),
				price_with_gps_in_words: numToWordUz(price_with_gps.toFixed(0)),
	...
	}, [application, newContract, metric, reset])
	...
	useEffect(() => {
		...
		const creditAmount = Number(application.tech.tech_price_with_vat) * (1 - Number(deposit_percentage) / 100)
	...
	return (
		<>
			<form
				...
				className='relative -mx-8 -my-6 flex flex-col overflow-hidden lg:flex-row'
			>
				<AdaptableCard
				...
				>
					...
					>
								...
								<Controller
									...
									render={({ field, fieldState: { invalid, error } }) => {
										const options: Option[] = branches
											? branches.map((branch) => ({
													label: branch.region.name_latin,
													value: branch.region.name_latin
												}))
											: []
														...
														onChange={(option) => {
															if (option) {
																...
																const branch = branches?.find(
																	(b) => b.region.name_latin === option.value
																)
																if (branch) {
						...
						<div className='flex flex-nowrap items-center gap-2 text-right mx-4 mb-4'>
								<Button
									size='md'
									className='grow'
...
export default NewContractsAddId
