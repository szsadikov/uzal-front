...
import { UserRegistryRow, UserRoleEnum } from '@/@types/user.types'
...
const UsersRegistryList = () => {
	const { t } = useTranslation()
	const { queries, setQueries } = useTableQueries<UserRegistryRow>({ page: 1, size: 10 })
	const [filters, setFilters] = useState({ is_active: true } as FilterQueries)
	...
	const {
		...
		queryFn: () =>UserService.getAllUsersRegistry<PaginatedResponse<UserRegistryRow[]>>(params),
		select: ({ data }) => data
	...
	return (
		<AdaptableCard className='h-full' bodyClass='h-full'>
			<div className='mb-4 items-center justify-between lg:flex'>
				<h3 className='mb-4 lg:mb-0'>{t('Клиент')}</h3>
				<UserTableTools
...
export default UsersRegistryList
