...
import TableFilter from './TableFilter' // ✅ qo‘shildi
...
const TableTools = ({
	...
	const handleExport = () => {
		...
		// ✅ faqat status === 'new' bo‘lganlar
		const onlyNew = payment_overdue_notice.results.filter(
			(r) => String(r.status).toLowerCase() === 'new'
		)

		if (!onlyNew.length) return

		const formatted = onlyNew.map((item) => ({
			ID: item.id,
		...
		exportToExcel(formatted, `NewPaymentNotices_${dayjs(new Date()).format('DD.MM.YYYY_HH-mm-ss')}`)
	}

	return (
		<div className='flex flex-col lg:flex-row lg:items-center'>
			<TableSearch value={search} onChange={onSearch} />
			{/* ✅ Filter ulandi va har doim status='new' bilan submit qiladi */}
			<TableFilter
				values={{ ...filters, status: 'new' }}
				onSubmit={(f) => onFilterSubmit({ ...f, status: 'new' })}
			/>

			<div className='flex flex-col space-y-2 lg:ml-2 lg:flex-row lg:space-y-0 lg:space-x-2'>
				<RequestAdd
					onSubmit={(delayed_time: string) => onDelay(delayed_time)}
				...
				{isLoading ? (
					...
					(payment_overdue_notice?.results?.length ?? 0) > 0 && (
						<Button
							className='mb-4 block md:mb-0 md:inline-block'
							size='sm'
							icon={<HiDownload />}
				...
				<Button
					...
					loading={isSending}
				>
					{isSending ? t('Отправляется...') : t('Отправить')}
				</Button>
			</div>
		</div>
	)
