...
const NewApplicationsAddPerform = ({ id, isOpen, setIsOpen, refetch }: Props) => {
	...
	const { mutateAsync: mutateAsyncUpdate, isPending: isPendingUpdate } = useMutation({
		...
		mutationFn: (data: FormModel) =>
			ContractService.updateApplications<ContractApplication, FormModel>(id, data),
		async onSuccess() {
			if (refetch) await refetch()

			toast.push(<Notification type='success' title={t('Заявка обновлена')} duration={2000} />, {
	...
	const { control, setValue, handleSubmit, reset } = useForm<FormModel>({
		mode: 'onChange'
	})
	...
	const onSubmit = async (data: FormModel) => {
		...
		const formData: FormModel = {
			...data,
			phone_number: data.phone_number.startsWith('998') ? data.phone_number : '998' + data.phone_number
		}

		await mutateAsyncUpdate(formData).finally(() => setIsOpen(false))
	}
	...
	useEffect(() => {
		if (application) {
			reset({
				...
				branch: application.branch.id,
				tech: application.tech.id,
				total_amount: Number(application.total_amount).toString(),
				sales: application.sales ? application.sales.id : null
			})
		}
	}, [application, reset])

	useEffect(() => {
		if (application && branches && branches.length) {
			const branch = branches.find((b) => b.id === application.branch.id)
...
export default NewApplicationsAddPerform
